...
SKOR: <0 ile 100 arası bir tam sayı; ruh sağlığı yüzdesi, düşük = daha komik/uçuk>
...
function raporAyristir(raw) {
  ...
  const bulunan = { teshis: '', skor: '', rapor: '' };
  let sonAnahtar = null;
  ...
  const skor = Math.max(0, Math.min(100, parseInt(bulunan.skor.replace(/[^0-9]/g, ''), 10)));
  ...
  return {
    ...
    skor: Number.isFinite(skor) ? skor : 50,
